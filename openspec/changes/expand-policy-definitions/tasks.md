...
- [x] 1.1 Build the distinct set of `definitionName` values from `RELEVANT_DENY_MODIFY_JSON`
      (dedup before issuing any `az` calls)
- [x] 1.2 For each distinct name, call `az policy definition show --name "<name>"` (read-only,
      wrapped with the existing `run_az_json` best-effort helper); on failure emit
      `{ name, error }` instead of aborting
- [x] 1.3 Normalize each resolved definition into `{ name, id, displayName, description,
...
- [x] 1.4 Assemble `POLICY_DEFINITIONS_DETAIL_JSON` from the resolved/error entries

## 2. Assignment detail attachment (bash)

- [x] 2.1 Build a `assignmentName -> assignment record` lookup from `ASSIGNMENTS_VISIBLE_JSON`
      (e.g. via `jq` `INDEX(.name)`)
- [x] 2.2 For each `RELEVANT_DENY_MODIFY_JSON` entry, look up its `assignmentName` in the map and
...
- [x] 2.3 Produce the updated `RELEVANT_DENY_MODIFY_JSON` with `assignmentDetail` inlined

## 3. Simplified constraint summary (bash)

- [x] 3.1 Implement a generic `jq` recursive walk over `policyRule.if` (and nested
      `allOf`/`anyOf`/`not`) that finds every object containing a `field` key and captures
      `{ field, operator, value }`
- [x] 3.2 Classify each captured condition into `resourceTypesAffected` / `namingRules` /
...
- [x] 3.3 Assemble one summary record per resolved definition:
...
- [x] 3.4 Add the summary array to the output JSON (alongside `policy.definitionsDetail`)

## 4. Mirror in PowerShell

- [x] 4.1 Port task group 1 (definition detail resolution) to `discover-subscription.ps1`,
...
- [x] 4.2 Port task group 2 (assignment detail attachment) using a hashtable keyed by
      `assignmentName`
- [x] 4.3 Port task group 3 (simplified constraint summary) as a recursive PowerShell function
...
- [x] 5.1 Run both scripts against the live test subscription; confirm `policy.definitionsDetail`
...
- [x] 5.2 Confirm at least one single-value array field in the new output (e.g. a definition with
...
- [x] 5.3 Confirm a deliberately-inaccessible or nonexistent definition name produces an
      `{ name, error }` entry without aborting the rest of discovery, in both scripts
- [x] 5.4 Update `scripts/README.md` to document the new `policy.definitionsDetail` array, the
      `assignmentDetail` field on shortlist entries, and the simplified constraint summary
- [x] 5.5 Mark all tasks in this file complete and run `openspec validate expand-policy-
      definitions --strict`
